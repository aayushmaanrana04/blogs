<script lang="ts">
    ...
    // SEO
    const pageTitle = "What Percent of X is Over? | Fragments";
    const pageDescription =
        "Track how much of the year, month, week, or day has passed. See time slip away, one percentage at a time.";
    const keywords =
        "year progress, time tracker, percent of year, time visualization, year percentage, how much of the year is over, time progress bar";

    let appSchema = $derived({
...
<svelte:head>
    ...
    <meta
        property="og:image"
        content={`${$page.url.origin}/og-what-percent.png`}
    />
            ...
            <div class="stats-row">
                <div class="stats-left">
                    ...
                    <div class="percent-label">
                        of the {contextLabel.toLowerCase()} is over
                    </div>
                </div>
                ...
                <div class="stats-right">
                    <div class="units-count remaining">
                        {(100 - mainProgress).toFixed(1)}%
                    </div>
    ...
    <div class="view-container" class:transitioning={isTransitioning}>
            ...
            <div class="century-view">
                <div class="decades-grid">
                    {#each getDecadesInCentury(currentYear) as decadeStart}
                        {@const progress = getDecadeProgressInCentury(
                            decadeStart,
                            currentYear,
                            now,
                        )}
                        {@const isCurrent =
                            Math.floor(currentYear / 10) * 10 === decadeStart}
                        {@const isPast =
                            decadeStart < Math.floor(currentYear / 10) * 10}
            ...
            <div class="decade-view">
                <div class="years-grid">
                    {#each getYearsInDecade(currentYear) as year}
                        {@const progress = getYearProgressInDecade(
                            year,
                            currentYear,
                            now,
                        )}
        ...
        {:else if zoomLevel === 3}
            ...
            <div class="month-view">
                <div class="month-card large">
                    <div class="month-name">
                        {getFullMonthName(currentMonth).toUpperCase()}
                    </div>
                    <div class="weekday-row">
        ...
        {:else}
            ...
            <div class="hours-view">
                ...
                <div class="hour-bar-label">
                    {hourProgress.toFixed(1)}% of this hour
                </div>
